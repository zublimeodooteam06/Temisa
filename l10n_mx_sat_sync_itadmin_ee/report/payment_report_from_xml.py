...
class ReportPaymentFromXMLFile(models.AbstractModel):
    ...
    def l10n_mx_edi_get_pago_etree(self, cfdi):
        cfdi_base = base64.decodebytes(cfdi)
        xml_data = etree.fromstring(cfdi_base)
        if xml_data.attrib['Version'] == '4.0':
            attribute = '//pago20:Pago'
            namespace = {'xsi':'http://www.w3.org/2001/XMLSchema-instance', 'cfdi':'http://www.sat.gob.mx/cfd/4', 'pago20': 'http://www.sat.gob.mx/Pagos20'}
        else:
            attribute = '//pago10:Pago'
            namespace = {'xsi':'http://www.w3.org/2001/XMLSchema-instance', 'cfdi':'http://www.sat.gob.mx/cfd/3', 'pago10': 'http://www.sat.gob.mx/Pagos'}
        Complemento = xml_data.find('cfdi:Complemento', namespace)
        if not Complemento:
            return None
        node = xml_data.find('cfdi:Complemento', namespace)
        node = node.find('pago20:Pagos', namespace)
        node =  node.find('pago20:Pago', namespace)
        return (node.attrib['FormaDePagoP'], node.attrib['FechaPago'], node.attrib['Monto'], node.attrib['MonedaP'], node.attrib['TipoCambioP'])
    ...
    def _get_report_values(self, docids, data=None):
        return {
            ...
            'l10n_mx_edi_get_pago_etree': self.l10n_mx_edi_get_pago_etree
            }
