...
#                    continue
    ...
    def import_customer_invoice(self, file_content, journal=False):
        ...
        if vendor_uuid != '':
            vendor_invoice = invoice_obj.search([('partner_id.vat', '=', partner_data.get('@Rfc'))])
            if vendor_invoice:
               vendor_order_exist = vendor_invoice.filtered(lambda inv: inv.l10n_mx_edi_cfdi_uuid == vendor_uuid.lower())
               if not vendor_order_exist:
                   vendor_order_exist = vendor_invoice.filtered(lambda inv: inv.l10n_mx_edi_cfdi_uuid == vendor_uuid.upper())
        ...
        invoice_vals = {
            ...
            'l10n_mx_edi_usage' : receptor_data.get('@UsoCFDI') if receptor_data.get('@UsoCFDI') != 'P01' else 'S01',
            ...
            'l10n_mx_edi_cfdi_uuid': timbrado_data.get('@UUID'),
    ...
    def import_supplier_invoice(self, file_content, journal=False):
        ...
        if vendor_uuid != '':
            vendor_invoice = invoice_obj.search([('partner_id.vat', '=', vendor_data.get('@Rfc'))])
            if vendor_invoice:
                vendor_order_exist = vendor_invoice.filtered(lambda inv: inv.l10n_mx_edi_cfdi_uuid == vendor_uuid.lower())
                if not vendor_order_exist:
                    vendor_order_exist = vendor_invoice.filtered(lambda inv: inv.l10n_mx_edi_cfdi_uuid == vendor_uuid.upper())
        ...
        invoice_exist = invoice_obj.with_context(ctx).create(invoice_vals)
        # invoice_exist.compute_taxes()
        invoice_exist.update({'l10n_mx_edi_cfdi_uuid': timbrado_data.get('@UUID')})
    ...
    def get_or_create_product(self, default_code, product_name, clave_unidad, unit_price, clave_producto, sale_ok=True, purchase_ok=False):
        ...
        if not product_exist:
            ...
            product_vals = {'default_code':default_code, 'name':product_name, 'standard_price' : unit_price, 'uom_id' : um_descripcion.id, 'unspsc_code_id' : sat_code.id,}
