...
class ReconcileVendorCfdiXmlBill(models.TransientModel):
    ...
    def action_reconcile(self):
        ...
        if self.typo_de_combante == 'RV':
           attachments = self.env['ir.attachment'].sudo().search([('id','in', selected_att_ids), ('creado_en_odoo','!=',True), ('cfdi_type','=', 'I')])
        elif self.typo_de_combante == 'RC':
           attachments = self.env['ir.attachment'].sudo().search([('id','in', selected_att_ids), ('creado_en_odoo','!=',True), ('cfdi_type','=', 'SI')])
        elif self.typo_de_combante != 'PI' and self.typo_de_combante != 'PE':
           ...
           attachments = self.env['ir.attachment'].sudo().search([('id','in', selected_att_ids), ('creado_en_odoo','!=',True)])
        ...
        if self.typo_de_combante in ['I','P']:
            ...
            payment_type = 'inbound'
        elif self.typo_de_combante in ['PI']:
            element_tag = 'Receptor'
            invoice_type = 'entry'
            payment_type = 'inbound'
        elif self.typo_de_combante in ['PE']:
            element_tag = 'Emisor'
            invoice_type = 'entry'
            payment_type = 'inbound'
        elif self.typo_de_combante in ['RV']:
            element_tag = 'Receptor'
            invoice_type = 'out_receipt'
            payment_type = 'inbound'
        elif self.typo_de_combante in ['RC']:
            element_tag = 'Emisor'
            invoice_type = 'in_receipt'
            payment_type = 'outbound'
...
#                 'methodo_pago': data.get('Comprobante',{}).get('@MetodoPago',''),
                ...
                'uso_cfdi': uso_data.get('@UsoCFDI') if uso_data.get('@UsoCFDI') != 'P01' else 'S01',
                }
            if self.typo_de_combante in ['P','SP']:
            ...
            elif self.typo_de_combante in ['PI','PE']:
                invoices = invoice_obj.search([('amount_total','=',total),('move_type','=', invoice_type)])
                if invoices:
        ...
        if self.typo_de_combante in ['P','SP']:
            ...
            action_id = 'l10n_mx_sat_sync_itadmin_ee.action_xml_entry_reconcile_view'
        else:
