        ...
        with zipfile.ZipFile(io.BytesIO(base64.b64decode(content[0]))) as z:
            for attachment_name in z.namelist():
                with z.open(attachment_name) as att_xml:
                    ...
                    if customer_documents:
                        if cfdi_type == 'P':
                            payment_exist_found = payment_obj.search([('partner_id.vat', '=', r_rfc), ('payment_type', '=', 'inbound'), ('company_id','=',self.id)])
                            if payment_exist_found:
                                for uu in [uuid.lower(), uuid.upper()]:
                                    payment_exist = payment_exist_found.filtered(lambda inv: inv.l10n_mx_edi_cfdi_uuid == vendor_uuid.lower())
                                    if payment_exist:
                                        vals.update({'creado_en_odoo': True, 'payment_ids': [(6, 0, payment_exist.ids)]})
                                        break
                        elif cfdi_type == 'E':
                            invoice_exist_found = invoice_obj.search([('partner_id.vat', '=', r_rfc), ('move_type', '=', 'out_refund'), ('company_id','=',self.id)])
                            if invoice_exist_found:
                                for uu in [uuid.lower(), uuid.upper()]:
                                    invoice_exist = invoice_exist_found.filtered(lambda inv: inv.l10n_mx_edi_cfdi_uuid == uu)
                                    if invoice_exist:
                                        vals.update({'creado_en_odoo': True, 'invoice_ids': [(6, 0, invoice_exist.ids)]})
                                        break
                        else:
                            invoice_exist_found = invoice_obj.search([('partner_id.vat', '=', r_rfc), ('move_type', '=', 'out_invoice'), ('company_id','=',self.id)])
                            if invoice_exist_found:
                                for uu in [uuid.lower(), uuid.upper()]:
                                    invoice_exist = invoice_exist_found.filtered(lambda inv: inv.l10n_mx_edi_cfdi_uuid == uu)
                                    if invoice_exist:
                        ...
                        if cfdi_type == 'SP':
                            payment_exist_found = payment_obj.search([('partner_id.vat', '=', r_rfc), ('payment_type', '=', 'outbound'), ('company_id','=',self.id)])
                            if payment_exist_found:
                                for uu in [uuid.lower(), uuid.upper()]:
                                    payment_exist = payment_exist_found.filtered(lambda inv: inv.l10n_mx_edi_cfdi_uuid == vendor_uuid.lower())
                                    if payment_exist:
                            ...
                            invoice_exist_found = invoice_obj.search([('partner_id.vat', '=', r_rfc), ('move_type', '=', 'in_refund'),  ('company_id','=',self.id)])
                            if invoice_exist_found:
                                for uu in [uuid.lower(), uuid.upper()]:
                                    invoice_exist = invoice_exist_found.filtered(lambda inv: inv.l10n_mx_edi_cfdi_uuid == uu)
                                    if invoice_exist:
                            ...
                            invoice_exist_found = invoice_obj.search([('partner_id.vat', '=', r_rfc), ('move_type', '=', 'in_invoice'), ('company_id','=',self.id)])
                            if invoice_exist_found:
                                for uu in [uuid.lower(), uuid.upper()]:
                                    invoice_exist = invoice_exist_found.filtered(lambda inv: inv.l10n_mx_edi_cfdi_uuid == uu)
                                    if invoice_exist:
